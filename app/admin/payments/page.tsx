  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        ...
        <p className="text-slate-300 text-sm mt-1">{payments?.length ?? 0} paid transactions</p>
      </div>
        ...
        <div className="bg-slate-900 border border-slate-700 rounded-xl p-5 mb-6">
          ...
          <div className="flex gap-3 flex-wrap">
            {Object.entries(monthlyRevenue).map(([month, revenue]) => (
              <div key={month} className="bg-slate-800 border border-slate-700 rounded-lg px-4 py-2.5">
                <p className="text-slate-300 text-xs">{month}</p>
                <p className="text-emerald-400 font-bold text-sm mt-0.5">RM {revenue.toFixed(2)}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-700 bg-slate-800">
                {['User', 'Plan', 'Amount', 'Date', 'Invoice Reference'].map(h => (
            ...
            <tbody>
              {(payments ?? []).map((p) => {
                ...
                return (
                  <tr key={p.id} className="border-b border-slate-800 hover:bg-slate-800/60 transition">
                    ...
                    <td className="px-5 py-3.5">
                      <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold ${
                        p.plan === 'bundle' ? 'bg-purple-900 text-purple-300' : 'bg-slate-700 text-slate-200'
                      }`}>{p.plan ?? 'single'}</span>
                    </td>
                    <td className="px-5 py-3.5 text-emerald-400 font-bold text-sm">RM {(p.amount / 100).toFixed(2)}</td>
          ...
          {(!payments || payments.length === 0) && (
            <p className="text-slate-400 text-sm text-center py-10">No payments yet</p>
          )}
        </div>
      </div>
    </div>
