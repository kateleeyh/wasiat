      ...
      <aside className="w-52 shrink-0 bg-slate-900 border-r border-slate-800 flex flex-col">
        ...
        <nav className="flex-1 px-3 py-4 space-y-1">
          {navItems.map(({ href, label, icon: Icon }) => (
            <Link key={href} href={href}
              className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm text-slate-400 hover:text-white hover:bg-slate-800 transition">
        ...
        <div className="px-3 py-4 border-t border-slate-800 space-y-1">
          <Link href="https://wasiathub.my" target="_blank"
            className="flex items-center gap-2 px-3 py-2 rounded-lg text-xs text-slate-500 hover:text-white hover:bg-slate-800 transition">
          ...
          <p className="text-slate-600 text-[10px] px-3 pt-1 truncate">{user.email}</p>
        </div>
      </aside>
